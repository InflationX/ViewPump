...
internal class `-ViewPumpLayoutInflater`(
    ...
    private val parentAndNameAndAttrsViewCreator: FallbackViewCreator =
        ParentAndNameAndAttrsViewCreator(this)

    // Reflection Hax
    private var setPrivateFactory = false

    private var storeLayoutResId = viewPump.isStoreLayoutResId
     ...
     */
    internal fun internalCloneInContext(newContext: Context): LayoutInflater {
        // Same context, use the same inflater
        if (newContext == context) return this
        return cloneInContext(newContext)
    }
    ...
    override fun setFactory(factory: Factory) {
        ...
        if (factory !is WrapperFactory) {
            super.setFactory(
                WrapperFactory(factory, viewPump)
            )
        } else {
    ...
    override fun setFactory2(factory2: Factory2) {
        ...
        if (factory2 !is WrapperFactory2) {
            ...
            super.setFactory2(
                WrapperFactory2(factory2, viewPump)
            )
        } else {
    ...
    private fun setPrivateFactoryInternal() {
        ...
        if (!viewPump.isReflection) return
        ...
        val setPrivateFactoryMethod =
            LayoutInflater::class.java.getAccessibleMethod("setPrivateFactory")

        setPrivateFactoryMethod.invokeMethod(
            this,
            PrivateWrapperFactory2(context as Factory2, viewPump, this)
        )
        setPrivateFactory = true
    }
    ...
    private fun createCustomViewInternal(
        ...
        if (!viewPump.isCustomViewCreation) return mutableView
        if (mutableView == null && name.indexOf('.') > -1) {
            if (isAtLeastQ) {
                mutableView = internalCloneInContext(viewContext).createView(name, null, attrs)
            } else {
